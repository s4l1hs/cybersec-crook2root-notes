...
```mermaid
flowchart LR
    A["Controlled input"] --> B["ELF Internals condition"]
...
A vendor-owned, instrumented laboratory build reproduces elf internals. The researcher demonstrates only a fixed marker, records mitigation behavior, patches the root cause, and preserves a regression input.
