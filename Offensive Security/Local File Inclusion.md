...
```http
GET /view?page=../../../../etc/hostname HTTP/1.1
```
...
In a multi-tenant enterprise service, an authorized assessor discovers local file inclusion on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
