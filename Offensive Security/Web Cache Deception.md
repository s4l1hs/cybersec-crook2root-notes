...
```http
GET /account/profile/canary.css HTTP/1.1
...
```text
X-Cache: MISS
Unauthenticated repeat: X-Cache: HIT
...
In a multi-tenant enterprise service, an authorized assessor discovers web cache deception on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
