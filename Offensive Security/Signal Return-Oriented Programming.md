---
title: "Signal Return-Oriented Programming"
...
```text
owned fixture: controlled instruction state
goal: invoke print_marker() only
```

### Expected vulnerable output

```text
stdout: C2R_CONTROL_FLOW_CANARY
exit status: 0
```
...
A vendor-owned, instrumented laboratory build reproduces signal return-oriented programming. The researcher demonstrates only a fixed marker, records mitigation behavior, patches the root cause, and preserves a regression input.
