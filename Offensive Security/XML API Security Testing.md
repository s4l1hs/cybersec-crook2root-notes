...
```mermaid
flowchart LR
    A["Controlled input"] --> B["XML API Security Testing condition"]
...
```xml
<transfer><amount>100</amount><amount>10000</amount></transfer>
```
...
```text
HTTP/1.1 200 OK
<acceptedAmount>10000</acceptedAmount>
```
...
In a multi-tenant enterprise service, an authorized assessor discovers xml api security testing on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
