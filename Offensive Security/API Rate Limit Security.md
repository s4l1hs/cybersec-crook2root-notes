...
```text
request: recovery-code x 21 for canary-user
window: 60 seconds
```
...
In a multi-tenant enterprise service, an authorized assessor discovers api rate limit security on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
