...
```http
GET /download?file=..%2f..%2f..%2fetc%2fhostname HTTP/1.1
```
...
In a multi-tenant enterprise service, an authorized assessor discovers path traversal on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
