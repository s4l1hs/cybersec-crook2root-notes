...
```mermaid
flowchart LR
    A["Controlled input"] --> B["SOAP Security Testing condition"]
...
```xml
<soap:Envelope><soap:Body><GetInvoice><id>INV-B-7</id></GetInvoice></soap:Body></soap:Envelope>
```
...
```text
HTTP/1.1 200 OK
<Invoice owner="TENANT-B">CANARY</Invoice>
```
...
In a multi-tenant enterprise service, an authorized assessor discovers soap security testing on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
