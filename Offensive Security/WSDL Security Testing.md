...
```mermaid
flowchart LR
    A["Controlled input"] --> B["WSDL Security Testing condition"]
...
In a multi-tenant enterprise service, an authorized assessor discovers wsdl security testing on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
