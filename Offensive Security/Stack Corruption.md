...
```text
buffer=32 bytes
input=80-byte cyclic canary
saved-return offset=40
```

### Expected vulnerable output

```text
crash: instruction pointer=0x4343434343434343
protected build: stack canary terminated process
```
...
A vendor-owned, instrumented laboratory build reproduces stack corruption. The researcher demonstrates only a fixed marker, records mitigation behavior, patches the root cause, and preserves a regression input.
