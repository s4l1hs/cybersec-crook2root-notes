...
**DNS Reconnaissance** is one atomic competency within **Reconnaissance & Attack Surface**. Learn the underlying mechanism before any product syntax: prerequisites, trust boundary, controllable state, security invariant, bounded evidence, business impact, and safe stopping point.

## Visual Attack Flow

```mermaid
flowchart LR
    A["Authorized prerequisite"] --> B["DNS Reconnaissance mechanics"]
