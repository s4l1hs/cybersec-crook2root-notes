...
```mermaid
flowchart LR
    A["Controlled input"] --> B["Spring Security Testing condition"]
...
In a multi-tenant enterprise service, an authorized assessor discovers spring security testing on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
