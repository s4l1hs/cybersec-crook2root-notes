---
title: "Payment Workflow Security"
...
**Payment Workflow Security** is an atomic discipline within **Business Logic & Workflow Security**. Mastery means identifying the controlling trust boundary, prerequisite identity, attacker-controlled value, vulnerable transformation, violated invariant, and smallest reproducible evidence. Do not confuse a product signature with the underlying mechanism.

## Visual Attack Flow

```mermaid
flowchart LR
    A["Controlled input"] --> B["Payment Workflow Security condition"]
...
```http
POST /orders/CANARY/refund HTTP/1.1
Idempotency-Key: alpha

(repeat with key beta)
```
...
refundTotal=200 orderTotal=100
```
...
In a multi-tenant enterprise service, an authorized assessor discovers payment workflow security on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
