...
```text
unsafe input: C2R-%p-%p
safe call: printf("%s", user_input)
```

### Expected vulnerable output

```text
unsafe output: C2R-0x7ffd0000-0x401210
safe output: C2R-%p-%p
```
...
A vendor-owned, instrumented laboratory build reproduces format string vulnerabilities. The researcher demonstrates only a fixed marker, records mitigation behavior, patches the root cause, and preserves a regression input.
