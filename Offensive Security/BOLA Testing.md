...
**BOLA Testing** is an atomic discipline within **Web Identity & Access Control**. Mastery means identifying the controlling trust boundary, prerequisite identity, attacker-controlled value, vulnerable transformation, violated invariant, and smallest reproducible evidence. Do not confuse a product signature with the underlying mechanism.

## Visual Attack Flow

```mermaid
flowchart LR
    A["Controlled input"] --> B["BOLA Testing condition"]
...
```http
GET /v1/documents/TENANT-B-CANARY HTTP/1.1
Authorization: Bearer <TENANT_A_CANARY>
```
...
In a multi-tenant enterprise service, an authorized assessor discovers bola testing on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
