...
```http
POST /v1/admin/users/canary/disable HTTP/1.1
Authorization: Bearer <ORDINARY_CANARY>
```
...
In a multi-tenant enterprise service, an authorized assessor discovers bfla testing on a synthetic customer workflow. The proof uses one canary object and stops before accessing production data.
