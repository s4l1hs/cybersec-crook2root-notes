...
**MITRE ATT&CK** is one atomic competency within **Methodologies & Frameworks**. Learn the underlying mechanism before any product syntax: prerequisites, trust boundary, controllable state, security invariant, bounded evidence, business impact, and safe stopping point.

## Visual Attack Flow

```mermaid
flowchart LR
    A["Authorized prerequisite"] --> B["MITRE ATT&CK mechanics"]
